...
async function register(req, res) {
  ...
  const mapRoleName = (name) => {
    const m = { ADMIN: 'Quản trị viên', SINHVIEN: 'Sinh viên', GIANGVIEN: 'Giảng viên' };
    return m[name] || name || '';
  };
  return res.status(201).json({
    ...
    data: {
      token,
      user: { id: user._id, username: user.username, email: user.email, roleId: role._id, roleName: mapRoleName(role.name) }
    }
  });
}
...
async function login(req, res) {
  ...
  return res.status(200).json({
    ...
    data: {
      token,
      user: { id: user._id, username: user.username, email: user.email, roleName: mapRoleName(user.roleId?.name) }
    }
  });
}
